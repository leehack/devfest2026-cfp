...
|---:|---|---|---|---|
| 1 | Owner creates/configures CFP | `VERIFIED-USER` creates an organization, then creates the call and becomes `EVENT-OWNER`; the owner configures event details, submission and confirmation forms, roles, intake window, email, and schedule inputs. | Organization membership authorises creation within its active-event limit; platform access alone grants no event data. The new call has one event owner, stable form keys, an explicit bounded intake window, and no public schedule. A pending committee grant queues one generic authenticated invitation; editing the role retains its invitation identity, revocation makes it unsendable, and a later re-invite gets a new identity. | `organizations.spec.ts`; `platform.spec.ts`; `submissionForm.spec.ts`; `confirmForm.test.ts`; `confirm.spec.ts`; `roles.spec.ts`; `staffNotifications.spec.ts`; `window.spec.ts` |
...
| Global platform owner/admin without an event role | No | No |
