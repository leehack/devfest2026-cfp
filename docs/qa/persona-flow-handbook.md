...
| `SPK-09` | `SPK-CONFIRMED` | `M0` | Before publication, inspect the own-placement card in My proposals. After publication, use Schedule, open the session detail, and download its ICS. | The working card is explicitly not public and contains only the speaker’s date, time, room, and language. Once public, the agenda, detail, frozen speaker snapshot, and stable ICS agree. | private own-placement card; public agenda with own talk; session detail/calendar action | Automated — `schedule.spec.ts`, “an accepted speaker confirms and follows their shared session into the published calendar file” |
...
| `REV-07` | `REVIEWER` + `REVIEWER-PENDING` + `REVIEWER-REVOKED` + `PLATFORM-GLOBAL-ONLY` | `MX` | Share a preview; inspect it as the active reviewer; attempt direct reads and writes as pending, revoked, and global-only identities. | The active committee member gets the confirmed agenda and public-safe custom items with Committee preview, Read-only, and Not public guidance. Tentative sessions are omitted. No reviewer may edit the draft. Pending, revoked, anonymous, and global-only identities receive no preview data. | active committee preview; tentative omission; read-only guidance; post-revocation denial | Automated — `schedule.spec.ts`, “committee preview follows active event membership and remains read only” |
...
| `ADM-07` | `EVENT-ADMIN` | `MX` | Configure days/rooms; place confirmed and tentative talks; share the confirmed preview; edit the draft; re-share; publish; take offline; then withdraw a scheduled talk. | The three stages show independent versions and audiences. Sharing omits tentative talks, snapshots the exact revision, and queues at most one actionable notification per changed speaker. Draft changes mark the shared preview stale and block publication until re-shared. Publishing promotes that exact release, warns while the CFP is open, queues no duplicate notice, and can be rolled back without deleting internal history. Withdrawal marks the stable shared/public projection cancelled and creates one cancellation notification. | private stage; share review; stale stages; open-CFP publish warning; live agenda; offline state; withdrawn session shown as cancelled | Automated — `schedule.spec.ts`; `scheduleUx.spec.ts`, “sharing and publishing have separate review steps and stale-version guidance”; schedule unit/rules tests |
